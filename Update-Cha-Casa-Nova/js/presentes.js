...
// Inicialização do Banco de Dados de Presentes no LocalStorage
let presentesDb = JSON.parse(localStorage.getItem('presentes_db') || '[]');
if (presentesDb.length === 0) {
  presentesDb = [...PRESENTES_DATA];
  localStorage.setItem('presentes_db', JSON.stringify(presentesDb));
}
...
function renderModalDonationForm(p) {
  ...
  if (donationForm) {
    donationForm.addEventListener('submit', (e) => {
      ...
      const dbIdx = presentesDb.findIndex(item => item.id === p.id);
      if (dbIdx !== -1) {
        presentesDb[dbIdx].status = 'reservado';
        presentesDb[dbIdx].quantidadeEscolhida = (presentesDb[dbIdx].quantidadeEscolhida || 0) + 1;
        presentesDb[dbIdx].progresso = 100;
        localStorage.setItem('presentes_db', JSON.stringify(presentesDb));
      }

      closeModal();
      showToast('Obrigado! Sua doação foi registrada com sucesso! 💛');
